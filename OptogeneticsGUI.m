        ...
        function CalibrationCallback(obj,src,~)
             switch src.Tag
                ...
                case 'RegisterCamSensorButton'
                    ...
                    obj.hardware.DMD.queueBuffer(uint8(zeros(width*height,1)));
                    ...
                    hi.CData = imgaccum;
                    valmax = nanmax(imgaccum(:));
                    threshold = valmin + (valmax-valmin)/2;
                    thresharea = sum(sum(imgaccum>threshold))/2;
                    ...
                    for p=1:size(pointscal,1)
                        ...
                        if (sum(bw(:)) >= thresharea) 
                            weightedx = camX.*bw;
                            weightedy = camY.*bw;
                            pointscam(p,:) = [sum(weightedx(:))./sum(bw(:))...
                                              sum(weightedy(:))./sum(bw(:))];
                        else
                            pointscam(p,:) = [NaN NaN];                  
                        end
                        
                        hi.CData = imgaccum;
                        ha.CLim = [valmin valmax];
                    ...
                    obj.affineTransform.Cam2Sensor = pointscam(tokeep,:)\pointscal(tokeep,:);
                    obj.affineTransform.Sensor2Cam = pointscal(tokeep,:)\pointscam(tokeep,:);
                    
                    obj.affineTransform.Cam2Sensor(:,3) = [0; 0; 1];
                    obj.affineTransform.Sensor2Cam(:,3) = [0; 0; 1];
             end
        end

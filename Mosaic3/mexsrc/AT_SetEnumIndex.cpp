                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    if (!mxIsScalar(prhs[0])) {
        mexErrMsgIdAndTxt("Optogenetics:AT_SetEnumIndex:InvalidInput","First input should be a scalar.");     
    }
    if (!mxIsChar(prhs[1])) {
        mexErrMsgIdAndTxt("Optogenetics:AT_SetEnumIndex:InvalidInput","Second input should be a string.");     
    }
    if (!mxIsScalar(prhs[2])) {
        mexErrMsgIdAndTxt("Optogenetics:AT_SetEnumIndex:InvalidInput","Third input should be a scalar.");     
    }
    ...
    ret = AT_SetEnumIndex(handle, feature, value);
    if (ret != 0) {
        mexWarnMsgIdAndTxt("Optogenetics:AT_SetEnumIndex:ReturnFailure","Library returned error %d",ret);
    }
